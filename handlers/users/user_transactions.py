...
from states.state_payment import StorageQiwi, StorageCrystal, StoragePayType
...
async def input_amount(call: CallbackQuery, state: FSMContext):
    ...
    if get_payment[5] == "True":
        if get_payment[0] != "None" and get_payment[1] != "None" and get_payment[2] != "None":
            await StoragePayType.here_input_pay_type.set()
            await call.message.answer("<b>💸 Выберите способ пополнения</b>",
                                      reply_markup=choose_pay_type_func(call.from_user.id))
            ...
            try:
                ...
                if pay_amount >= min_input_qiwi:
                    ...
                    await message.answer(send_message,
                                         reply_markup=create_pay_func(send_requests,
...
# Способы оплаты
###########################
@dp.callback_query_handler(state=StoragePayType.here_input_pay_type)
async def get_pay_type(call: CallbackQuery, state: FSMContext):
    ...
    if pay_type == "qiwi":
        ...
        await StorageCrystal.here_input_crystal_amount.set()
        await call.message.answer("💵 Введите сумму для пополнения средств 💎")
    await bot.delete_message(call.from_user.id, call.message.message_id)
...
###########################
# Создание платежа
@dp.message_handler(state=StorageCrystal.here_input_crystal_amount)
async def create_crystal_pay(message: types.Message, state: FSMContext):
    if message.text.isdigit() and int(message.text) >= 0:
        crystal = CrystalPay(data.config.crystal_name, data.config.crystal_secret)
        link = crystal.generate_pay_link(message.text, redirect="https://t.me/ranshop_bot")
        await message.answer("🎈 Ссылка на оплату сгенерирована:\n"
                            f"✔ ID платежа: {link[0]}\n"
                            f"📎 Ссылка: <a href='{link[1]}'>нажмите для пополнения счёта</a>",
                             reply_markup=create_pay_func(send_requests=link[1], receipt=link[0],
        ...
        await bot.delete_message(message.chat.id, del_msg.message_id)
        await message.answer(f"❌ <b>Неверная сумма пополнения</b>\n"
                             f"▶ Мин. сумма пополнения: <code>{1}руб</code>\n"
                             f"💵 Введите сумму для пополнения средств 💎")
